...
class CNNStem(nn.Module):
    def __init__(self, in_ch=3, c3_dim=128, c4_dim=256):
        super().__init__()
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(96, c3_dim, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(c3_dim),
            nn.ReLU(inplace=True),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(c3_dim, c4_dim, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(c4_dim),
            nn.ReLU(inplace=True),
        )
    def forward(self, x):
        x = self.conv1(x)
...
class SAMRoad(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # ---- Stage C/D: FPN + Stage D CNN stem (Phase-1 uses only P4 for map_decoder) ----
    ...
    def forward(self, rgb, graph_points, pairs, valid):
        ...
        if self.config.USE_SAM_DECODER:
            ...
            if self.use_fpn:
                c4_sam = image_embeddings  # SAM S4 (1/16)
                # Stage D modules are created in __init__ when USE_CNN_STEM=True
                if getattr(self, 'use_cnn_stem', False):
                    c3, c4_cnn = self.cnn_stem(x)
                    c4_fused = self.fuse_conv(torch.cat([c4_cnn, c4_sam], dim=1))
                    c5 = self.c5_down(c4_fused)
                    p3, p4, p5 = self.fpn(c3, c4_fused, c5)
                else:
                    c4 = c4_sam
                    c5 = F.max_pool2d(c4, kernel_size=2, stride=2)
                    c3 = F.interpolate(c4, scale_factor=2, mode='nearest')
                    p3, p4, p5 = self.fpn(c3, c4, c5)
                mask_logits = self.map_decoder(p4)
            else:
    ...
    def infer_masks_and_img_features(self, rgb):
        ...
        if self.config.USE_SAM_DECODER:
            ...
            if self.use_cnn_stem:
                # ===== Stage D =====
                ### STAGE_D_FWD_DEBUG_ONCE ###
                if not hasattr(self, "_stageD_fwd_debug_printed"):
                    self._stageD_fwd_debug_printed = True
                    print("[StageD-FWD] x:", tuple(x.shape), x.device)
                    print("[StageD-FWD] S4:", tuple(image_embeddings.shape), image_embeddings.device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            # Only log tables if the active logger supports it (e.g., WandbLogger). CSVLogger does not.
